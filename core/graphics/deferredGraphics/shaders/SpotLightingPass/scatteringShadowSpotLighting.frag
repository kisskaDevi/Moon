...
void main()
{
    ...
    vec4 fragLightProj = light.projView * vec4(position.xyz,1.0f);
    float ShadowFactor = outsideSpotCondition(fragLightProj.xyz / fragLightProj.w,type) ? 0.0f: shadowFactor(shadowMap, fragLightProj);
    ...
    vec4 textureLightColor = distribusion * texture(lightTexture, (fragLightProj.xy / fragLightProj.w) * 0.5f + 0.5f);
    ...
    outColor += LightScattering(50, light.view, light.proj, light.projView, light.position, lightColor, projview, eyePosition, glPosition, lightTexture, shadowMap, depthMap, type);
