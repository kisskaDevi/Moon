...
{
    float sigma = textureSize(bloomSampler, 0).x;
    vec2 textel = 1.0 / textureSize(bloomSampler, 0);
    vec3 Color = texture(bloomSampler, TexCoord).xyz /sqrt(pi*sigma);
    int h = 20;
    for(int i=1;i<h+1;i+=2)
    {
	float I1 = 1.0f/sqrt(pi*sigma) * exp( -(i*textel.x*i*textel.x)/sigma);
	float I2 = 1.0f/sqrt(pi*sigma) * exp( -((i+1)*textel.x*(i+1)*textel.x)/sigma);
	float x = (I1*i+I2*(i+1))*textel.x/(I1+I2);
	float I = 1.0f/sqrt(pi*sigma) * exp(-(x*x)/sigma);
